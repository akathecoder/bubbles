...
export function useBubblesProfile(ensName: string) {
  const { data: textRecords, isLoading, error } = useEnsTexts({
    name: ensName,
    keys: [
      ENS_CONFIG.TEXT_KEYS.DESCRIPTION,
      ENS_CONFIG.TEXT_KEYS.PREFERRED_PAYMENT,
      ENS_CONFIG.TEXT_KEYS.BUBBLES_PROFILE,
      ENS_CONFIG.TEXT_KEYS.BUBBLES_AVATAR,
      ENS_CONFIG.TEXT_KEYS.BUBBLES_CREATED,
    ],
    enabled: !!ensName,
  });
  ...
  const profileData = textRecords ? {
    ...
    createdAt: textRecords.find(r => r.key === ENS_CONFIG.TEXT_KEYS.BUBBLES_CREATED)?.value || '',
    bubblesProfile: (() => {
      const profileText = textRecords.find(r => r.key === ENS_CONFIG.TEXT_KEYS.BUBBLES_PROFILE)?.value;
      try {
  ...
  return {
    ...
    isLoading,
    error,
    hasProfile: !!profileData?.bubblesProfile,
  };
}